...
class CosmosDbEngine(TargetResourceEngine):
    ...
    def get_app_settings_identity(self, binding: Binding) -> List[tuple]:
        ...
        default_settings = [
            (AppSettingType.KeyValue, 'AZURE_COSMOS_RESOURCEENDPOINT', '{}.outputs.endpoint'.format(self.module_name)),
        ]
    ...
    def get_app_settings_secret(self, binding: Binding) -> List[tuple]:        
        custom_keys = dict() if binding.customKeys is None else binding.customKeys
        default_settings = [
            (AppSettingType.KeyVaultReference, 'AZURE_COSMOS_CONNECTIONSTRING', '{}.outputs.keyVaultSecretUri'.format(self.module_name)),
        ]
